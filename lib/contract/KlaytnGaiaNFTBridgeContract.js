    ...
    async watch() {
        ...
        setInterval(async () => {
            ...
            for (const event of sendEvents) {
                this.fireEvent("SendNFTs", event.returnValues[0], bignumber_1.BigNumber.from(event.returnValues[1]), event.returnValues[2], event.returnValues[3], event.returnValues[4], event.returnValues[5], event.returnValues[6]);
            }
            const receiveTokenEvents = await this.getReceiveNFTsEvents(prevBlock, currentBlock);
            for (const event of receiveTokenEvents) {
                this.fireEvent("ReceiveNFTs", event.returnValues[0], bignumber_1.BigNumber.from(event.returnValues[1]), event.returnValues[2], event.returnValues[3], event.returnValues[4], event.returnValues[5], event.returnValues[6]);
            }
